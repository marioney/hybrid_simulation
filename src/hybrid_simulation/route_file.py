...
def generate_route_file_dmaking(route_file_path, n_scenario):
    ...
    with open(route_file_path, "w") as routes:
        ...
        if n_scenario == 1:
            print('    <vehicle id="%s" type="ego-vehicle" route="route00" depart="0" departSpeed="max" color="1,1,1"/>' % ego_vehicle_id, file=routes)
            print('    <vehicle id="car_1" type="Car_90" route="route03" depart="0" departSpeed="max"/>', file=routes)

        if n_scenario == 2:
            print('    <vehicle id="%s" type="ego-vehicle" route="route03" depart="1" color="1,1,1"/>' % ego_vehicle_id, file=routes)
            print('    <vehicle id="car_1" type="Car_110" route="route01" depart="1" />', file=routes)
            print('    <vehicle id="car_2" type="Car_130" route="route00" depart="1" />', file=routes)
            print('    <vehicle id="car_3" type="Car_90" route="route05" depart="1" />', file=routes)
            # print('    <vehicle id="%s" type="ego-vehicle" route="route03" depart="0" departSpeed="max" color="1,1,1"/>' % ego_vehicle_id,
            #    file=routes)
            # print('    <vehicle id="car_1" type="Car_110" route="route01" depart="0" departSpeed="max"/>', file=routes)
            # print('    <vehicle id="car_2" type="Car_130" route="route00" depart="0" departSpeed="max"/>', file=routes)
